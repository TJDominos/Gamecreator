...
// Modify feature grid map
const featureGridRegex = /<div className="feature-grid">\s*\{aiIterationFeatures\.map\(\(\{ image, title, text \}, index\) => \(\s*<article className="feature-card feature-card--with-image" key=\{title\}>/m;

if (code.match(featureGridRegex)) {
  code = code.replace(featureGridRegex, `<div className="feature-grid">
  ...
  const articleCloseRegex = /<\/article>\s*\)\)/m;
  code = code.replace(articleCloseRegex, `</article>\n                </div>\n              ))`);
}
